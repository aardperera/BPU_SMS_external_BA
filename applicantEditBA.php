...
if (isset($_POST['btnSubmit'])) {
   ...
   /* $queryqua = "SELECT * FROM  ba_qualifications WHERE applicantID='$studentID'";
    ...
    for($i = 1 ; $i <= $no_qua ; $i++){
            ...
            if($degree == ''){
                $query = "DELETE from ba_qualifications WHERE id = '$id'" ;
            }
            else{
                $query = "UPDATE ba_qualifications SET degree='$degree', institute='$institute', indexno='$indexno', `year`='$year', subjects='$subjects', class='$class', contacts='$contacts' WHERE id = '$id'" ;
            }
            $result = $db->executeQuery($query);        
    }
    ...
    for ($i = ++$no_qua; $i <= 10; $i++) {
        if (isset($_POST['q_institute' . $i])) {
            ...
            $query = "INSERT INTO ba_qualifications SET degree='$degree', institute='$institute', indexno='$indexno', `year`='$year', subjects='$subjects', class='$class', contacts='$contacts', studentID='$studentID'";
            $result = $db->executeQuery($query);
        }
    }
    */

        $ol=$db->cleanInput($_POST['txtol']);
		$al=$db->cleanInput($_POST['txtal']);
		$diplomaE=$db->cleanInput($_POST['txtdiplomaE']);
        $diplomaB=$db->cleanInput($_POST['txtdiplomaB']);
        $diplomaP=$db->cleanInput($_POST['txtdiplomaP']);
        $diplomaS=$db->cleanInput($_POST['txtdiplomaS']);
        $diplomaD=$db->cleanInput($_POST['txtdiplomaD']);
        $diplomaT=$db->cleanInput($_POST['txtdiplomaT']);

		$Darmacharya=$db->cleanInput($_POST['txtdharma']);
        $PrachinaM=$db->cleanInput($_POST['txtpraM']);
        $PrachinaP=$db->cleanInput($_POST['txtpraP']);
		$Degree=$db->cleanInput($_POST['txtdegree']);
        $Ctest=$db->cleanInput($_POST['txtctest']);
        $bp=$db->cleanInput($_POST['txtbp']);
        $bc=$db->cleanInput($_POST['txtbc']);
        $pali=$db->cleanInput($_POST['txtpali']);
        $san=$db->cleanInput($_POST['txtsan']);
        $eng=$db->cleanInput($_POST['txteng']);
        $arch=$db->cleanInput($_POST['txtarch']);
        $rs=$db->cleanInput($_POST['txtrs']);
        $sin=$db->cleanInput($_POST['txtsin']);
        $esin=$db->cleanInput($_POST['txtesin']);
        $bcon=$db->cleanInput($_POST['txtbcon']);
        $tamil=$db->cleanInput($_POST['txttamil']);
        $ctamil=$db->cleanInput($_POST['txtctamil']);


        $queryq = "UPDATE ba_qualifications SET OL='$ol',AL='$al', HDEnglish='$diplomaE', HDBud='$diplomaB', HDPali='$diplomaP',
        HDSans='$diplomaS',	HDDaham='$diplomaD',HDTamil='$diplomaT', Darmacharya='$Darmacharya', PrachinaM='$PrachinaM', PrachinaP='$PrachinaP',Degree='$Degree',Ctest='$Ctest' WHERE applicantID='$studentID' ";
   ////print $queryq;
            $result = $db->executeQuery($queryq);


            $querysub = "UPDATE ba_subjects SET bp='$bp',bc='$bc', pali='$pali', san='$san', eng='$eng',
                arch='$arch',rs='$rs', esin='$esin', bcon='$bcon', tamil='$tamil', sin='$sin',ctamil='$ctamil' WHERE applicantID='$studentID'";
               // print  $querysub;
           //print  $querysub ;
          //print ("***************************");
           //print $querysub;
                    $result = $db->executeQuery($querysub);


   header("location:applicantAdminBA.php?page=$pageNum&select=$select&acyear=$ayear");
}
...
?>
<form method="post" action="applicantEditBA.php?studentID=<?php echo $studentID; ?>"
    onsubmit="return validate_form(this);" class="plain">
    <table>
        <tr>
            <td></td>
            <td>Applicant ID : </td>
            <td colspan="1"><input name="txtStudentID" type="text" value="<?php echo $row['applicantID']; ?>"
        ...
        <tr>
            <td height="26">Name in Sinhala  : </td>
            <td colspan="3"><input name="txtNameSinhala" type="text" value="<?php echo $row['nameSinhala']; ?>"
                    maxlength="400" style="width:500px" required /></td>
        </tr>
        ...
        echo 'checked'; ?> /> Languages Studies &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
                &nbsp; &nbsp; &nbsp;
	  ...
	  <table border="0">
		...
		<td></td><td>Buddist Philosophy</td> <td><input name="txtbp" type="checkbox" value="1" <?php if ($rowsub['bp'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
		<tr>
		<td></td><td>Buddist Culture</td> <td><input name="txtbc" type="checkbox" value="1" <?php if ($rowsub['bc'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Pali</td> <td><input name="txtpali" type="checkbox" value="1" <?php if ($rowsub['pali'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Sanskrit</td> <td><input name="txtsan" type="checkbox" value="1" <?php if ($rowsub['san'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>English</td> <td><input name="txteng" type="checkbox" value="1" <?php if ($rowsub['eng'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Archaeology</td> <td><input name="txtarch" type="checkbox" value="1" <?php if ($rowsub['arch'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Religious Study</td> <td><input name="txtrs" type="checkbox" value="1" <?php if ($rowsub['rs'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Sinhala</td> <td><input name="txtsin" type="checkbox" value="1" <?php if ($rowsub['sin'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Tamil</td> <td><input name="txtctamil" type="checkbox" value="1" <?php if ($rowsub['ctamil'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td>2.</td><td>Elective Subjects</td> <td> </td>
		</tr>
		<tr>
		<td></td><td>Sinhala</td> <td><input name="txtesin" type="checkbox" value="1" <?php if ($rowsub['esin'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Buddist Councelling</td> <td><input name="txtbcon" type="checkbox" value="1" <?php if ($rowsub['bcon'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Tamil</td> <td><input name="txttamil" type="checkbox" value="1" <?php if ($rowsub['tamil'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
	  ...
	  <table border="0">
		<tr>
		<td>1.</td><td>O/L</td> <td><input name="txtol" type="checkbox" value="1" <?php if ($rowq['OL'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
		<tr>
		<td>2.</td><td>A/L</td> <td><input name="txtal" type="checkbox" value="1" <?php if ($rowq['AL'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td>3.</td><td>Higher Diploma</td> <td> </td>
		</tr>
		<tr>
		<td></td><td>English</td> <td><input name="txtdiplomaE" type="checkbox" value="1" <?php if ($rowq['HDEnglish'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Buddhisum</td> <td><input name="txtdiplomaB" type="checkbox" value="1" <?php if ($rowq['HDBud'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Pali</td> <td><input name="txtdiplomaP" type="checkbox" value="1" <?php if ($rowq['HDPali'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Sanskrit</td> <td><input name="txtdiplomaS" type="checkbox" value="1" <?php if ($rowq['HDSans'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>DahamSarasawiya</td> <td><input name="txtdiplomaD" type="checkbox" value="1" <?php if ($rowq['HDDaham'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td></td><td>Tamil</td> <td><input name="txtdiplomaT" type="checkbox" value="1" <?php if ($rowq['HDTamil'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
		<tr>
		<td>4.</td><td>Dharmacharya</td> <td><input name="txtdharma" type="checkbox" value="1" <?php if ($rowq['Darmacharya'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
		<tr>
		<td>5.</td><td>Prachina</td> <td> </td>
		</tr>
		<tr>
		<td> </td><td>Prachina Maddhyam</td> <td><input name="txtpraM" type="checkbox" value="1" <?php if ($rowq['PrachinaM'] == '1') echo 'checked'; ?>  /></td>
		</tr>
		<tr>
		<td> </td><td>Prachina Praramba</td> <td><input name="txtpraP" type="checkbox" value="1" <?php if ($rowq['PrachinaP'] == '1') echo 'checked'; ?>  /></td>
		</tr>
        <tr>
		<td>6.</td><td>Degree</td> <td><input name="txtdegree" type="checkbox" value="1" <?php if ($rowq['Degree'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
        <tr>
		<td>7.</td><td>Common Test</td> <td><input name="txtctest" type="checkbox" value="1" <?php if ($rowq['Ctest'] == '1') echo 'checked'; ?>  /> </td>
		</tr>
