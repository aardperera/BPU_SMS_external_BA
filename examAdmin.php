...
$db = new DBOperations();
  ...
  $query = "SELECT effortID,exameffort.indexNo,student_a.nameEnglish AS student_a,subject.nameEnglish AS subject,exameffort.acYear,mark1,mark2,marks,grade,effort FROM exameffort JOIN crs_enroll ON exameffort.indexNo=crs_enroll.indexNo JOIN student_a ON crs_enroll.studentID=student_a.studentID JOIN subject ON exameffort.subjectID=subject.subjectID";
  //print $query;
  ...
  function filterQuery($acYear,$subject,$student)
  {
	...
	if ($acYear<>0)
	{
		$subQuery = " WHERE exameffort.acYear='".$acYear."' AND crs_enroll.yearEntry = '".$acYear."' "; // (1,_,_)
		if ($subject<>0)
		{
 ...
 <form method="post" action="" class="plain">
  <table style="margin-left:8px" class="panel">
  	<tr>
        ...
        <td>&nbsp;</td>
        <td><input name="btnEnterResults" type="button" value="Enter First Mark Results" class="button" onclick="document.location.href ='examEnterResults.php'" /></td>
   <td>&nbsp;</td>
        <td><input name="btnEnterResults" type="button" value="Enter Second Marks Results" class="button" onclick="document.location.href ='examEnterResults2.php'" /></td>
		<td>&nbsp;</td>
        <td><input name="btnEnterResults" type="button" value="Withholding Students" class="button" onclick="document.location.href ='examWH.php'" /></td>
	</tr>
   </table>
   <table style="margin-left:8px" class="panel">
    <tr>
        ...
        <td>
            <select name="lstStudent" id="lstStudent" onchange="this.form.submit();">
                <?php
				$result = $db->executeQuery("SELECT DISTINCT indexNo FROM crs_enroll where courseID = '".$_SESSION['courseId']."'");
				if ($db->Row_Count($result)>0)
				{
					echo "<option value='0'>All</option>";
					while ($row= $db->Next_Record($result))
