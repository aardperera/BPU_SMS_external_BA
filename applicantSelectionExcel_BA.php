...
$query = "SELECT * FROM ba_applicant WHERE acyear='" . $acyear . "'";

if ($select != '') {
    // Modify the query to filter by selection status
    if ($select == 'Selected') {
        $query .= " AND selection = 'Selected'";
    } elseif ($select == 'Pending') {
        $query .= " AND (selection IS NULL OR selection = '')"; // Filter for both NULL and empty selections
    } elseif ($select == 'Rejected') {
        $query .= " AND selection = 'Rejected'";
    }
}
...
while($row = $db->Next_Record($result)){
    ...
    $row['address'] = $row['addressE1'] . "\r\n" . $row['addressE2']. "\r\n" . $row['addressE3'];
    $row['addressSinhala'] = $row['addressS1']. "\r\n" .  $row['addressS2']. "\r\n" .  $row['addressS3'];
...
$columns=['applicantID', 'title', 'nameEnglish', 'initials', 'nameFullEnglish','nameSinhala', 'address','addressSinhala', 'city', 'postalcode', 'districtstate', 'country', 'nic', 'mobile', 'email', 'passport', 'birthday', 'age', 'gender', 'civilStatus', 'citizenship', 'religion', 'employment', 'employer', 'medium', 'field', 'quli','comsub','elesub','certificate', 'photos', 'payment', 'selection', 'selection_date', 'c_payment', 'c_payment_date'];
