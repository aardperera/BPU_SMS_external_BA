...
$db = new DBOperations();
  ...
  {
	$regNo = $db->cleanInput($_GET['courseID']);
	$delQuery = "DELETE FROM course_combination WHERE courseID ='courseID'";
  ...
  $query = "SELECT b.courseCode,a.courseID,a.subcrsID,a.description FROM crs_sub as a,course as b where a.CourseID=b.courseID";
...
?>
   ...
   <td><input name="btnDetails" type="button" value="Details" onclick="document.location.href ='coursesubdetails.php?combinationID=<?php echo $row['subcrsID']?>'"  class="button" style="width:60px;"/></td>
   
      <td><input name="btnEdit" type="button" value="Edit" onClick="document.location.href ='coursesubedit.php?combinationID=<?php echo $row['subcrsID'] ?>'" class="button" style="width:60px;"/></td>
        <td><input name="btnDelete" type="button" value="Delete" class="button" onClick="if (MsgOkCancel()) document.location.href ='coursesubAdmin.php?cmd=delete&courseID=<?php echo $row['courseID'] ?>'" style="width:60px;" /></td>
