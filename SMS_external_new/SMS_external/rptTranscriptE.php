...
$db = new DBOperations();
	...
	$courseName = mysql_result($result,0,'nameEnglish');
	...
	$result = $db->executeQuery("SELECT codeEnglish,nameEnglish,level,grade,marks,effort FROM exameffort JOIN subject ON exameffort.subjectID=subject.subjectID WHERE indexNo='$indexNo' AND effortID IN (SELECT effortID FROM exameffort JOIN (SELECT subjectID,MAX(marks) as marks FROM exameffort GROUP BY subjectID) AS temp_ee ON exameffort.subjectID=temp_ee.subjectID AND exameffort.marks=temp_ee.marks) ORDER BY level");
