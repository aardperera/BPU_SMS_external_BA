...
$userName = "root";
$password = "";
$databaseName = "sms_external";

function $db->executeQuery($query)
{	
	...
	$result = @ $db->executeQuery($query,$connection) or die(mysql_errno($connection).": ".mysql_error($connection));
...
{	
	...
	if ($num_quaries>0)
	{
		...
		{
			$result[$i] = $db->executeQuery($quries[$i]."; ") or die(mysql_errno($connection).": ".mysql_error($connection));
		}
		//if (in_array(false,$result)) $db->executeQuery("ROLLBACK; ");
