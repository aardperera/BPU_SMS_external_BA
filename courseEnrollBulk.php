...
if (isset($_POST['btnSubmit']))
	...
	if($subcrsID==7 || $subcrsID==8)
	{
		...
		$errorstuid = '';
	$testDate = date('Y-m-d');
	$query1234 = "SELECT a.Enroll_id, a.regNo, b.title, b.nameEnglish FROM `crs_enroll` AS a, student_a AS b WHERE a.subcrsID = $subcrsID AND a.yearEntry = $lstYearEntry AND a.regNo = b.studentID order by a.indexNo ;";
	...
	while ($row1234 = $db->Next_Record($result1234))
		...
		if(isset($_POST['stu'.$row1234['Enroll_id']]))
		{
			...
			$query11234 = "SELECT regNo FROM `crs_enroll` WHERE subcrsID = '".($subcrsID+1)."' AND yearEntry = '".($lstYearEntry)."' AND regNo = (SELECT regNo FROM crs_enroll WHERE Enroll_id = '$enrlid');";
			...
			$query12345 = "SELECT a.subjectID, b.codeEnglish FROM `subject_enroll` AS a, subject AS b WHERE a.Enroll_id = '$enrlid' AND a.subjectID = b.subjectID AND b.notMap ='y' ;";
			//print $query12345;
		...
		{
			$query1234 = "SELECT a.Enroll_id, a.regNo, b.title, b.nameEnglish, a.indexNo, a.studentID FROM `crs_enroll` AS a, student_a AS b WHERE a.subcrsID = $subcrsID AND a.yearEntry = $lstYearEntry AND a.regNo = b.studentID;";
	$result1234 = $db->executeQuery($query1234);
	while ($row1234 = $db->Next_Record($result1234))
		  {
		if(isset($_POST['stu'.$row1234['Enroll_id']]))
...
<table class="searchResults">
    ...
    <tr>
    	<td>Acadameic Year : </td><td><select name="lstYearEntry" id="lstYearEntry"   onChange="document.form.submit()">
        	<?php
				for ($i=2018;$i<=2050;$i++)
				{
...
//if (mysql_num_rows($pageResult)>0){ 
	...
	$query1234 = "SELECT a.Enroll_id, a.regNo, b.title, b.nameEnglish,a.indexNo FROM `crs_enroll`AS a, student_a AS b WHERE a.subcrsID = $subcrsID AND a.yearEntry = $lstYearEntry AND a.regNo = b.studentID order by a.indexNo;";
	$result1234 = $db->executeQuery($query1234);
...
<h5>Enrolled Student List </h5>
	<tr>
    	<th>#</th><th>Student ID</th><th>Index No</th><th>Title</th><th>Student Name</th><th></th>
    </tr>
...
<?php
	$cnt = 1;
		  while ($row1234 = $db->Next_Record($result1234))
		  {
			  ?>
	<tr>
    	<td><?php echo $cnt;?></td><td><?php echo $row1234['regNo'];?></td><td><?php echo $row1234['indexNo'];?></td><td><?php echo $row1234['title'];?></td><td><?php echo $row1234['nameEnglish'];?></td>
