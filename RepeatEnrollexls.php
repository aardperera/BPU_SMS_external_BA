...
$yearEntry = isset($_GET['academicYear']) ? $_GET['academicYear'] : '';
$courseID = 5;
$subcrsID = 7;
//$yearEntry='2019';

// Build the SQL query to fetch data from crs_enroll and ba_applicant tables
// $query = "SELECT ce.studentID, ce.regNo, ce.indexNo, sa.title, sa.nameEnglish, sa.nameSinhala, sa.addressE1, sa.addressS1, sa.contactNo, sa.email FROM crs_enrollr ce INNER JOIN subject_enrollr se ON ce.Enroll_id = se.Enroll_id INNER JOIN student_a sa ON sa.studentID = ce.studentID WHERE `yearEntry` = '$yearEntry' LIMIT 10";
$query = "SELECT ce.studentID, ce.regNo, ce.indexNo, sa.title, sa.nameEnglish, sa.nameSinhala, CONCAT(sa.addressE1, ' ', sa.addressE2, ' ', sa.addressE3) AS englishAddress, CONCAT(sa.addressS1, ' ', sa.addressS2, ' ', sa.addressS3) AS sinhalaAddress, sa.contactNo, sa.email FROM crs_enrollR ce INNER JOIN subject_enrollR se ON ce.Enroll_id = se.Enroll_id INNER JOIN student_a sa ON sa.studentID = ce.studentID WHERE `yearEntry` = '$yearEntry' ";
...
//            exit;
        ...
        $enroll_test = $db->Next_Record($db->executeQuery("SELECT COUNT(ID) AS count1 FROM `subject_enrollR` where `subjectID` = '$subjectID' AND `Enroll_id` = (SELECT `Enroll_id` FROM `crs_enrollR` where `regNo`='$regNo' AND `yearEntry` ='$yearEntry' AND `courseID` = '$courseID' AND `subcrsID` = '$subcrsID')"));
//        print_r($enroll_test);
//        exit();
...
$subjectEnrollQuery = "SELECT Enroll_id, subjectID FROM subject_enrollR";
...
$titles = ['Student ID', 'Registration No', 'Index No', 'Title', 'Name (English)', 'Name (Sinhala)', 'Address (English)', 'Address (Sinhala)', 'Contact No', 'Email'];
...
$columns = ['studentID', 'regNo', 'indexNo', 'title', 'nameEnglish', 'nameSinhala', 'englishAddress', 'sinhalaAddress', 'contactNo', 'email'];
