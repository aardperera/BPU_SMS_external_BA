 ...
 // $resultall5 = $db->executeQuery($queryall5);
  //while ($row5=  $db->Next_Record($resultall5)){
	//  $subjectID=$row5['subjectID'];
	   ...
	   $checkboxvalue=$Enrollid;
	   
	      $queryexist3 = "Select * from attend_admission where admissionID='$admissionID'";
		$resultexist3 = $db->executeQuery($queryexist3);
		$value3= $db->Row_Count($resultexist3);
			 if($value3==0){
	   if ( isset( $_POST[ 'checkBox' . $checkboxvalue] ) ) {
	
	   print "INSERT INTO attend_admission (admissionID,attendance)VALUES('$admissionID',''1')";
	  
$resultinsert = $db->executeQuery("INSERT INTO attend_admission (admissionID,attendance)VALUES($admissionID,'1')");
print "INSERT INTO attend_admission (admissionID,subjectID,attendance)VALUES('$admissionID','$subjectID','1')";
 ...
 if ( isset( $_POST[ 'checkBox' . $checkboxvalue] ) ) {
$resultinsert = $db->executeQuery("UPDATE attend_admission SET attendance='1' WHERE admissionID=$admissionID ");
	  
	  }

else{
$resultinsert = $db->executeQuery("UPDATE attend_admission SET attendance='0' WHERE admissionID=$admissionID ");
}
}
}
...
<br/>
 ...
 <tr>
  ...
  <table class="searchResults">
	<tr>
    	<th>Index No</th><th>Name</th> <th>Attendance</th>
    </tr>
...
?>
 <td><center><input type='checkBox' name="checkBox<?php echo  $enrollID ?>"  id="checkBox<?php echo  $enrollID?>" value="checkBox<?php echo  $enrollID ?>"  ></center> </td> </tr> 
    ...
    ?> 
        <td><center><input type='checkBox' name="checkBox<?php echo  $enrollID ?>"  id="checkBox<?php echo  $enrollID?>" value="checkBox<?php echo  $enrollID ?>" checked></center> </td> </tr> 
