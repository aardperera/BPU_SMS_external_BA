...
<?php
 ...
 // 	include('authcheck.inc.php');
	...
	if (isset($_POST['btnSubmit']))
	{
		...
		$compulsary = $db->cleanInput($_POST['compulsary']);
	...
	$valueee=$row2['subjectID'];
		...
		$query = "UPDATE subject SET courseID='5',codeEnglish='$codeEnglish', nameEnglish='$nameEngslih', codeSinhala='$codeSinhala', nameSinhala='$nameSinhala', faculty='$faculty', level='$level', semester='$semester', creditHours='$chours',suborder='$suborder',Compulsary='$compulsary',notMap='$notMap' WHERE subjectID='$subjectID'";
	...
	$subjectID = $db->cleanInput($_GET['subjectID']);
	$query = "SELECT *,a.nameEnglish as EName FROM crs_subject as c,`subject`as a,course as b WHERE a.subjectID='$subjectID' AND  b.courseID=c.courseID and a.subjectID=c.subjectID ";
	//print $query;
	$result = $db->executeQuery($query);
...
<table class="searchResults">
    ...
    <tr>
    	<td>Name (English) : </td><td><input name="txtNameEnglish" type="text" value="<?php echo $row['EName'] ?>" style="width:300px"/></td>
    </tr>
    <tr>
	...
	<tr>
	  ...
	  <td><select name="notMap">
        <option <?php if ($row['notMap']=='y') echo "selected='selected'"; ?> value="y">Yes</option>
		   <option <?php if ($row['notMap']!='y') echo "selected='selected'"; ?> ></option>
