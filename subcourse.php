...
$db = new DBOperations();
	...
	if (isset($_POST['btnSubmit']))
	{
		...
		$newvalu=$rowvalu+1;
		$query = "INSERT INTO crs_sub SET  CourseID='$courseID', subcrsID='$newvalu', Description='$description' , course_by='$courseby'";
...
<form method="post" name="form1" id="form1" action="" onSubmit="return validate_form(this);" class="plain">
  <table class="searchResults">
    <tr> 
      ...
      <option value="">---</option>
          <?php
			$query = "SELECT courseID,nameEnglish,courseCode FROM course;";
			$result = $db->executeQuery($query);
    ...
    <tr> 
	 ...
	 <tr> 
      <td>Course By  : </td>
      <td><select id="Courseby" name="Courseby"  >
      <option value="">---</option>
          <?php
			 echo '<option value="E">Exam</option>'; 
			  echo '<option value="R">Reserch</option>'; 
			?>
        </select>
        <script type="text/javascript" language="javascript">
		document.getElementById('CourseID').value="<?php if(isset($courseID)){echo $courseID;}?>";
