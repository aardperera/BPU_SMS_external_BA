  ...
  if (isset($_POST['btnSubmit']))
	{
		...
		while ($row=  $db->Next_Record($resultall))
		{
			...
			if ($marks!=""){
				...
				$resultinsert = $db->executeQuery("INSERT INTO exameffort (indexNo,subjectID,mark1,acYear) VALUES ('$indexNo', $SubjectID, '$marks', $acyear)");
 ...
 <table width="230" class="searchResults">
    <tr> 
      ...
      <td width="91"><select id="courseID" name="courseID" onchange="document.form1.submit()" >
          <?php
			$query = "SELECT id,code FROM course WHERE `id` ='" . $_SESSION['courseId'] . "' ";
    ...
    <tr>
	  ...
	  <select id="SubjectID" name="SubjectID" onchange="document.form1.submit()" >
              <?php
					...
					$sql = "SELECT  a.id,a.code_english,a.name_english,a.name_sinhala,a.level,a.credit_hours FROM subject as a, course as b WHERE b.id=a.course_id and b.id='" . $_SESSION['courseId'] . "'  ";
                    $result = $db->executeQuery($sql);

                    while ($rowg = $db->Next_Record($result)) {
                        echo '<option value="' . $rowg['id'] .'">'. $rowg['code_english'] .'--'. $rowg['name_english'] .'</option>';
	...
	<tr>
      ...
      <td><label>
     	  <?php
								...
								$sql="SELECT distinct year FROM enrollment order by year";
								$result = $db->executeQuery($sql);
								//echo '<option value="all">Select All</option>';
