  ...
  if (isset($_POST['btnSubmit']))
	{
		...
		///print $indexNo[1];
		$queryall = "Select * from subject_enrollR as s, crs_enrollR as c, paymentEnrollR as p where s.subjectID='$SubjectID' and s.Enroll_id=c.Enroll_id and c.yearEntry='$acyear' and p.payment1='1' and p.payment2='1' and c.Enroll_id=p.enroll_id ";
		//print $queryall;
		...
		while ($row=  $db->Next_Record($resultall))
		{
			...
			//print $marks ;
			//print 'g';
			$queryexist = "Select * from exameffortR where subjectID='$SubjectID' and indexNo='$indexNo' and acYear='$acyear' and exYear='$exyear'";
			...
			if ($marks!=""){
				...
				//print 'uuu';
				//print $mark;
				//print "INSERT INTO exameffortR (indexNo,subjectID,mark1,acYear)VALUES($indexNovalue,$SubjectID,$marks,$acyear)";
				$resultinsert = $db->executeQuery("INSERT INTO exameffortR (indexNo,subjectID,mark1,acYear,exYear) VALUES ('$indexNo', $SubjectID, '$marks', '$acyear','$exyear')");
				...
				//print 'ttt';
				//print "UPDATE exameffortR set mark1=$mark where effortID='$effortvalue'";
				$resultupdate = $db->executeQuery("UPDATE exameffortR set mark1='$marks' where effortID='$effortvalue'") ;
				}
			}
		}
...
<br/>
 <table width="230" class="searchResults">
	...
	<tr>
      ...
      <td><label>
     	  <?php
								...
								$sql="SELECT distinct yearEntry FROM crs_enrollR order by yearEntry";
								$result = $db->executeQuery($sql);
								//echo '<option value="all">Select All</option>';
	...
	<tr>
      ...
      <td><label>
		<?php  
		 $query123 = "Select * from exameffortR where subjectID='$SubjectID'  and acYear='$acyear' limit 1"; 
		
		 $result123 = $db->executeQuery($query123);
	 	 
		  while ($row123=$db->Next_Record($result123))
		  {
			$exyear=$row123['exYear'];
		  }
		
		?>
	  <input name="exyear" id="exyear" type="text" value="<?php  echo $exyear; ?>" /><?php  echo $exyear1; ?></td>
							
							 <script>
								document.getElementById('exyear').value = "<?php if(isset($exyear)) echo $exyear;?>";
							</script>
 </label>
		</td>
...
if (isset($_POST['SubjectID']) && isset($_POST['acyear']))
{
 ...
 $queryall = "Select * from subject_enrollR as s, crs_enrollR as c, paymentEnrollR as p where s.subjectID='$SubjectID' and s.Enroll_id=c.Enroll_id and c.yearEntry='$acyear' and p.payment1='1' and p.payment2='1' and c.Enroll_id=p.enroll_id order by c.indexNo ";
// print $queryall;
  $resultall = $db->executeQuery($queryall);
  ...
  {
	  $indexNo=$row['indexNo'];
	 $query12 = "Select * from exameffortR where subjectID='$SubjectID' and indexNo='$indexNo' and acYear='$acyear'";  
	 //print $query12;
							$result12 = $db->executeQuery($query12);
	  $row12=$db->Next_Record($result12);
?>
	<tr>
		...
		<td>
			<input type="text" id="txtMarks<?php echo $row['indexNo'] ?>" name="txtMarks<?php echo $row['indexNo'] ?>" value="<?php if($row12) echo $row12['mark1'] ?>" maxlength="3" oninput="this.value = this.value.replace(/[^0-9ABMD]/g, '').replace(/(\..*)\./g, '$1'); verifymark('txtMarks<?php echo $row['indexNo'] ?>', this.value);" required onchange="verifymark('txtMarks<?php echo $row['indexNo'] ?>', this.value)" />
