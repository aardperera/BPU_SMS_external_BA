...
$db = new DBOperations();
	...
	if (isset($_POST['btnSubmit']))
	{
		...
		$countryCode = $db->cleanInput($_POST['txtCountryCode']);
...
<form method="post" action="studentEdit.php?studentID=<?php echo $studentID; ?>" onsubmit="return validate_form(this);" class="plain">
  <table class="searchResults">
    <tr> 
      <td>StudentID : </td>
      <td><input name="txtStudentID" type="text" id="txtStudentID" value="<?php echo $row['studentID']; ?>" readonly="readonly" /></td>
    </tr>
    <tr>
      <td>NIC No. :</td>
      <td><input name="txtNic" id="txtNic" type="text" placeholder="123456789V or 123456789012" oninput="this.value = this.value.replace(/[^0-9V]/g, '').replace(/(\..*)\./g, '$1'); verifynic(this.value);" required maxlength="12" value="<?php echo $row['nic']; ?>" style="width:300px"/>
    ...
    <tr> 
      ...
      <td><input name="txtAddressE1" type="text" value="<?php echo $row['addressE1'] ?>" style="width:300px"/>
        <br/>
    ...
    <tr> 
      ...
      <td><input name="txtAddressS1" type="text" value="<?php echo $row['addressS1'] ?>" style="width:300px"/>
        <br/>
    ...
    <tr> 
      <td>Contact No. : </td>
      <td><input style="width:50px" id="txtCountryCode" name="txtCountryCode" type="text" maxlength="6" oninput="this.value = this.value.replace(/[^0-9+]/g, '').replace(/(\..*)\./g, '$1'); verifycontrycode(this.value);" value="<?php echo $row['countryCode'] ?>" required placeholder="+xxxx"/>
		<span class="validity"></span>  
		<input name="txtContactNo" id="txtContactNo" type="text" maxlength = "10" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/(\..*)\./g, '$1'); verifyconatct(this.value);" required value="<?php echo $row['contactNo'] ?>" />
		<span class="validity"></span>
		</td>
    </tr>
    <tr> 
...
<script>
	function verifyconatct(value) {
		var val = document.getElementById("txtContactNo").value;
		if (val.length == 9 && val.charAt(0) != "0") {
			document.getElementById("txtContactNo").setCustomValidity("");
			...
			document.getElementById("txtContactNo").setCustomValidity("Invalid field");
		}
	}
</script>
...
<script>
	function verifycontrycode(value) {
		var val = document.getElementById("txtCountryCode").value;
		if (val.charAt(0) == "+" && val.length > 1) {
			document.getElementById("txtCountryCode").setCustomValidity("");
		}
		else {
			console.log("wrong country code");
			//document.getElementById("txtCountryCode").value = "";
			document.getElementById("txtCountryCode").setCustomValidity("Invalid field");
		}
	}
</script>
