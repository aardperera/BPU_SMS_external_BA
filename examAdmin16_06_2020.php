...
$db = new DBOperations();
  ...
  $query = "SELECT effortID,exameffort.indexNo,student.nameEnglish AS student,subject.nameEnglish AS subject,exameffort.acYear,marks,grade,effort FROM exameffort JOIN crs_enroll ON exameffort.indexNo=crs_enroll.indexNo JOIN student ON crs_enroll.studentID=student.studentID JOIN subject ON exameffort.subjectID=subject.subjectID";
   ...
   <table style="margin-left:8px" class="panel">
    <tr>
        ...
        <td>
            ...
            <?php
				$result = $db->executeQuery("SELECT indexNo FROM crs_enroll");
				if ($db->Row_Count($result)>0)
				{
					echo "<option value='0'>All</option>";
					while ($row= $db->Next_Record($result))
...
<br/>
  <table class="searchResults">
	<tr>
    	<th>Index No.</th><th>Student</th><th>Subject</th><th>Ac. Year</th><th>Result</th><th>Effort</th><th colspan="2"></th>
    </tr>
