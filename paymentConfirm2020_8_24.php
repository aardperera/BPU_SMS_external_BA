...
$resultinsert = $db->executeQuery("INSERT INTO paymentEnroll (enroll_id,payment1)VALUES($Enrollid,'1')");
//print 'a';
}
else{
$resultinsert = $db->executeQuery("INSERT INTO paymentEnroll (enroll_id,payment1)VALUES($Enrollid,'0')");
//print 'b';
}
if ( isset( $_POST[ 'checkBox2' . $Enrollid] ) ) {
$resultinsert = $db->executeQuery("UPDATE paymentEnroll SET payment2='1' where enroll_id='$Enrollid' ");
//print 'c';
}
else{
$resultinsert = $db->executeQuery("UPDATE paymentEnroll SET payment2='0' where enroll_id='$Enrollid' ");
//print 'd';
}
}
else{
if ( isset( $_POST[ 'checkBox1' . $Enrollid] ) ) {
$resultinsert = $db->executeQuery("UPDATE paymentEnroll SET payment1='1' where enroll_id='$Enrollid'");
//print 'e';

}
else{
$resultinsert = $db->executeQuery("UPDATE paymentEnroll SET payment1='0' where enroll_id='$Enrollid'");
//print 'f';
}
if ( isset( $_POST[ 'checkBox2' . $Enrollid] ) ) {
$resultinsert = $db->executeQuery("UPDATE paymentEnroll SET payment2='1' where enroll_id='$Enrollid' ");
...
?>
	<tr>
		...
		<?php 
		 $Enrollid = $row[ 'Enroll_id' ];
		 $queryexist1 = "Select * from paymentEnroll where enroll_id='$Enrollid' and payment1='1'";
		 $queryexist2 = "Select * from paymentEnroll where enroll_id='$Enrollid' and payment2='1'";
			
			$resultexist1 = $db->executeQuery($queryexist1);
			$resultexist2 = $db->executeQuery($queryexist2);
			
			 $value1= $db->Row_Count($resultexist1);
			  $value2= $db->Row_Count($resultexist2);
			 if($value1==0){
		
		?>
		
		<td><center><input type='checkBox' name="checkBox1<?php echo $row[0] ?>"  id="checkBox1<?php echo $row[0] ?>" value="checkBox1<?php echo $row[0] ?>"  ></center> </td>
<?php }
else{
   ?>
   <td><center><input type='checkBox' name="checkBox1<?php echo $row[0] ?>"  id="checkBox1<?php echo $row[0] ?>" value="checkBox1<?php echo $row[0] ?>"  checked="checked" ></center> </td>
    
	<?php  
	}
	 if($value2==0){
	
	?>  <td><center><input type='checkBox' name="checkBox2<?php echo $row[0] ?>" id="checkBox2<?php echo $row[0]?>"  ></center></td>

        <?php
		}
		else{
		?>
        <td><center><input type='checkBox' name="checkBox2<?php echo $row[0] ?>" id="checkBox2<?php echo $row[0]?>" checked="checked"  ></center></td>
		<?php
		}
		?>
	</tr>
