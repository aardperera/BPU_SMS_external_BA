...
//$query.= " WHERE studentID LIKE '".$_SESSION['sfcourseCode']."' ";
  ...
  // counting the offset
	...
	$pageResult2 = $db->executeQuery($pageQuery);
	$row =  $db->Next_Record($pageResult);
	echo $row['studentID'] ;
       echo $row['title'] ;
 echo $row['nameEnglish']; 
...
<?php
	...
	if (isset($_POST['btnSubmit']))
	{
		...
		header("location:studentAdmin.php?newcourseID=$courseID&combinationID=$combinationID");
	}
...
$valurows= $db->Row_Count($pageResult2);
echo $valurows;

if ($db->Row_Count($pageResult2)>0){ ?>
<br/>
  <table class="searchResults">
	<tr>
...
//$query.= " WHERE studentID LIKE '".$_SESSION['sfcourseCode']."' ";
 ...
 echo $row['nameEnglish']; 
  ...
  while ($row2 =  $db->Next_Record($pageResult2))
  {
?>
	<tr>
        <td><?php echo $row2['studentID'] ?></td>
        <td><?php echo $row2['title'] ?></td>
		<td><?php echo $row2['nameEnglish'] ?></td>
        <td><?php echo $row2['addressE1'].$row2['addressE2'].$row2['addressE3'] ?></td>
