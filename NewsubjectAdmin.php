<?php
 ...
 <?php
  ...
  if (isset($_GET['cmd']) && $_GET['cmd']=="delete")
  {
	...
	$delQuery = "DELETE FROM Newsubject WHERE subjectID='$subjectID'";
  ...
  $query = "SELECT  a.subjectID,a.codeEnglish,a.Compulsary,a.nameEnglish,a.nameSinhala,a.level,a.creditHours,a.suborder FROM Newsubject as a, course as b, crs_subject as c WHERE b.courseID=c.courseID and a.subjectID=c.subjectID and c.courseID='5'";
   //print $query;
   if ($_SESSION['courseId']!=0)
		  {
		  //$query = $query. " and c.courseID='5' orderby c.subcrsid";
		  }
		  //print  $query;
  ...
  $query = $query." ORDER BY c.subcrsid, a.suborder";
  ...
  function filterQuery($faculty,$level)
  {
	...
	$subQuery = $subQuery." ORDER BY c.subcrsid, a.suborder";
...
?>
  ...
  <table style="margin-left:8px" class="panel">
  	<tr>
    	<td><input name="btnNew" type="button" value="New" onclick="document.location.href = 'NewsubjectNew.php';" class="button" style="width:60px;" /></td>
        <td>&nbsp;</td>
        <td>Faculty</td>
        <td>
		...
		<td><input name="btnNew" type="button" value="Map Subjects" onclick="document.location.href = 'subjectMap.php';" class="button" style="width:100px;" /></td>
  	</tr>
  </table>
<?php 
...
<br/>
  <table class="searchResults">
	<tr>
    	...
    	<th>Compulsary</th>
    	<th>Name English</th><th>Name Sinhala</th><th>Credit Hours</th><th>Order</th><th colspan="3"></th>
    </tr>
...
?>
	<tr>
		...
		<td width=200><?php echo $row['nameSinhala'] ?></td>
