...
$db = new DBOperations();
	...
	$subjectID = $db->cleanInput($_POST['SubjectID']);
	    ...
	    $courseID = $_POST['CourseID'];
		$subcrsID= $_POST['subcrsID'];
		$queryeee = "SELECT * FROM crs_sub WHERE courseID='$courseID' and subcrsID='$subcrsID'";
		//print $queryeee;
	$resulteee = $db->executeQuery($queryeee);
	$roweee =  $db->Next_Record($resulteee);
	 $valueee=$roweee['id'];
		//print  $valueee;
		$compulsary = $_POST['compulsary'];
		...
		$query = "INSERT INTO crs_subject SET  CourseID='$courseID', subcrsID='$valueee',SubjectID='$SubjectID',compulsary='$compulsary'";
